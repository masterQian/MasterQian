...
#define MasterQianModuleVersion 20240107ULL
...
namespace MasterQian::Parser {
	namespace details {
		...
		inline mqbytes Write(mqbytes data, T&& arg, Args&&... args) noexcept {
			data = WriteImpl(data, std::forward<T>(arg));
			if constexpr (sizeof...(Args) > 0ULL) data = Write(data, std::forward<Args>(args)...);
			return data;
		}
	}
	...
	/// 配置对象
	/// </summary>
	struct Config : protected std::unordered_map<std::wstring, Bin, Meta::isomerism_hash, Meta::isomerism_equal> {
		using PointT = Bin::Bin::PointerT;
		using ConstPointT = Bin::Bin::ConstPointerT;
		using SizeT = Bin::Bin::SizeT;
		using BaseT = std::unordered_map<std::wstring, Bin, Meta::isomerism_hash, Meta::isomerism_equal>;
	public:
		Config() = default;
		...
		void load(BinView bv) noexcept {
			...
			for (SizeT i{ }; i < count; ++i) {
				...
				try_emplace(std::move(key), std::move(value));
			}
		}
