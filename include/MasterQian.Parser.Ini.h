...
namespace MasterQian::Parser {
	namespace details {
		inline mqcstr IniScanLine(mqcstr pData, mqcstr* p1, mqcstr* p2, mqcstr* p3, mqcstr* p4) {
			mqcstr pStart{ pData };
	...
	/// 节视图
	/// </summary>
	struct SectionView : protected std::unordered_map<std::wstring_view, std::wstring_view> {
	...
	/// ini视图
	/// </summary>
	struct IniView : protected std::unordered_map<std::wstring_view, SectionView> {
	protected:
	...
	/// ini配置文件
	/// </summary>
	struct Ini : public std::unordered_map<std::wstring, Section> {
		Ini() = default;
